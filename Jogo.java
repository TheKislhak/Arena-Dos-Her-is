...
public class Jogo {
	...
	public static void main(String[] args) {
		...
		Personagem mago = new Mago("Frieren");
		Personagem gigante = new Gigante("Bertolt");
		Personagem guerreiro = new Guerreiro("Jonathan Joestar");
		Personagem arqueiro = new Arqueiro ("Hanzo");
		...
		while (!tabuleiro.checarVencedor()) {
			for (Personagem  p : personagens) {
				if (p.getVida() > 0) {
					...
					tabuleiro.movimentaPersonagem(p, novaPosicao);
				}
			}
		}
		scanner.close();
